...
SELECT * FROM USER_CONSTRAINTS; -- CONSTRAINTS - 제약조건
...
-- MEMBER 테이블 생성해보기
CREATE TABLE "MEMBER" ( ---- 쌍따움표 사용 시 "소대문자 그대로"
	MEMBER_ID VARCHAR2(20),
	MEMBER_PWD VARCHAR2(20),
	MEMEBR_NAME VARCHAR2(30),
	MEMBER_SSN CHAR(14), -- '991213-1234567'
	ENROLL_DATE DATE DEFAULT SYSDATE
);

-- 만든 테이블 확인해보기
SELECT * FROM "MEMBER"; -- 조회시에는 "" 존재 여부 상관없음

-- 2. 컬럼에 주석 달기
-- [표현식]
-- COMMENT ON COLUMN 테이블명.커럼명 IS '주석내용';

COMMENT ON COLUMN MEMBER.MEMBER_ID IS '회원 아이디';
COMMENT ON COLUMN MEMBER.MEMBER_PWD IS '회원 비밀번호';
COMMENT ON COLUMN MEMBER.MEMEBR_NAME IS '회원 이름';
COMMENT ON COLUMN MEMBER.MEMBER_SSN IS '회원 주민등록번호';
COMMENT ON COLUMN MEMBER.ENROLL_DATE IS '회원 가입일';

-- MEMBER 테이블에 샘플 데이터 삽입
-- INSERT INTO 테이블병 VALUES(값1, 값2..);
...
-- DEFAULT 설정된 컬럼이라고 삽입/수정 시 DEFAULT만 작성해야 하는 것이 아니다
-- 컬럼 데이터 타입에 맞춘 데이터 삽입이 가능하다.


-- 가입일 -> INSERT 시 미작성 하는 경우 DEFAULT 값이 반영되는지 확인
-- INSERT INTO 테이블명 (컬럼명1, 컬럼명2) VALUES(값1, 값2);

INSERT INTO "MEMBER" (MEMBER_ID, MEMBER_PWD, MEMEBR_NAME)
VALUES ('MEM03', '1Q2W3E', '이지연');

SELECT * FROM MEMBER;
-- MEMBER_SSN컬럼과 ENROLL_DATE 컬럼에 값을 삽입하지 않았는데,
-- MEMBER_SSN에는 NULL, ENROLL_DATE의 경우 DEFAULT로 설정한 값이 삽입되는 것을 확인 가능.
...
-- 제일 앞에 0이 있는 경우 이를 자동으로 제거함.
-- > 전화번호, 주민등록번호처럼 숫자로만 되어있는 데이터라도
-- 0으로 시작될 가능성이 있으면 CHAR, VARCHAR2같은 문자열 타입으로 사용해야 함.
...
--USER_ID VARCHAR2(20) UNIQUE,	-- 컬럼 레밸(자약조건명 미지정) 에 작성하는 방법
--USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE,	-- 컬럼 레밸 (제약조건명 지정)	방법
	USER_ID VARCHAR2(20), 
	USER_PWD VARCHAR2(20),
	USER_NAME VARCHAR2(30),
	GENDER VARCHAR2(10),
	PHONE VARCHAR2(30),
	EMAIL VARCHAR2(50),	-- 테이블 레밸에 제약조건 작성할 경우 마지막 컬럼에 , 있어야함.
	--  테이블 레밸 구간	
	--UNIQUE(USER_ID) -- 테이블 레벨(제약조건명 미지정)
	CONSTRAINT USER_ID_U UNIQUE(USER_ID) -- 테이블 레밸(제약조건명 지정)
);
...
-- USER_ID 컬럼은 UNIQUE 제약 조건이 걸려있어서 중복된 값 삽입 불가.  
...
1, NULL, 'PASS01', '홍길동', '남자', 
'010-1234-5678', 'hong_gd@or.kr');
--> 아이디에 NULL값 중복 삽입 가능 확인 (UNIQUE 제약 조건 - NULL값은 중복 삽입 가능)
...
-- UNIQUE 복합키
-- 두 개 이상의 컬럼을 묶어서 하나의 UNIQUE 제약조건을 설정하는 것
...
-- 테이블의 한 행 한 행에 대한 식별자(사용자번호, 학번..) 역할을 함
...
1, 'USER02', 'PASS01', '홍길동', '남자', 
'010-1234-5678', 'hong_gd@or.kr');
-- 둘 다 중복됬을 때만 위배임. 유저번호 중복이지만 아이디가 다르므로 위배가 안됨.
...
-- 컬럼레벨에 작성할 경우 FOREIGN KEY 작성 안 해도 됨.
...
-- 부모테이블 == 참조할 테이블 == 레퍼런스 테이블 (참조 대상이 되는 테이블)
...
CREATE TABLE USER_USED_FK (
	...
	REFERENCES USER_GRADE /*(GRADE_CODE)*/ -- 컬럼레밸
								-- (컬럼명)부분 작성 안 할 시 USER_GRADE 테이블의로 PK(기본키) 자동 참조함.
-- 테이블 레벨
	--, CONSTRAINT GRADE_CODE_FK FOREIGN KEY(GRADE_CODE)
	-- REFERENCE USER_GRADE
...
-- USER_GRADE(부모/참조테이블)에 
-- 10 이라는 GRADE_CODE가 존재하므로 가능
...
-- 제공되는 값 외에는 NULL만 사용 가능한 조건으로 인해 NULL 사용 가능



INSERT INTO USER_USED_FK VALUES (
4, 'USER05', 'PASS05', '윤봉길', '남자',
'010-6666-7777', 'yoon_jk@or.kr', 50);
-- ORA-00001: 무결성 제약 조건(KH.SYS_C007384)에 위배됩니다
...
-- 자식 레코드가 발견되었습니다 : 자식 테이블의 컬럼(FOREIGN KEY로 지정된 컬럼)에서 30을 사용하고 있다는 의미

-- GRADE_CODE 중 20은 자식에서 사용되지 않고 있으므로 삭제가 가능함.
DELETE FROM USER_GRADE WHERE GRADE_CODE = 20;
SELECT * FROM USER_GRADE; -- 20 삭제된 것 확인 가능.
ROLLBACK; -- 20 돌아도로록 되돌리기
...
-- 부모테이블인 USER_GRADE2에서 GRADE_CODE가 10 삭제 잘 됨.
...
INSERT INTO USER_GRADE23 VALUES(30, '특별회원');
...
-- '남' 또는 '여'만 삽입 가능하도록 설정해둬서
--- 이 외에 값이 들어오면 체크 제약 조건 위배되어 에러 발생함!
