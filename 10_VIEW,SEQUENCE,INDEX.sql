 ...
 * 	2) VIEW를 이용한 DML(INSERT,UPDATE,DELETE)이 가능한 경우도 있지만(하지 않는 것을 권장)
 ...
 * 
 *  1) OR REPLACE 옵션 : 
 * 		기존에 동일한 이름의 VIEW가 존재하면 이를 변경(덮어씀)
 * 		없으면 새로 생성
 * 
 *  2) FORCE | NOFORCE 옵션 : 
...
-- 12버전 이후 부터?? C##을 붙여야 하는데 그런 규칙 무시하겠다는 구문 ????
--> 계정명이 언급되는 상황에서는 선행으로 이 구문을 필수적으로 진행하는 것이 좋다

-- 관리자 SYS계정이 KH계정에 VIEW 생성 권한 부여(계정명이 언급됨)
...
-- VIEW 구문 실행
CREATE VIEW V_EMP
AS SELECT * FROM EMPLOYEE; -- AS 다음에 작성된 코드의 RESULT SET을 그대로 복제하여 가상테이블 V_EMP 생성
-- > VIEW 생성 구문 기초 문법.

SELECT * FROM V_EMP;

-- 사번, 이름, 부서명, 직급명 조회하기 위한 VIEW 생성
--> OR REPLACE(기존에 동일한 이름의 VIEW가 존재하면 이를 변경(덮어씀)
-- 없으면 새로 생성) 옵션을 사용
...
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID) -- 부서 없는 직원도 조회되도록 LEFT JOIN
ORDER BY 1 ASC;
...
SELECT * FROM V_DCOPY2;	-- D0, L2 값 삽입 확인

SELECT * FROM DEPT_COPY2;	-- 원본 테이블에 D0, NULL, L2 값 자동 삽입됨
-- VIEW에 INSERT를 수행했지만
...
--> 따라서 의도되지 않은 NULL은 존재하지 않게 해야한다.
 ...
 * 
 *	무결성이 보장된다 : 중복 데이터가 없어야 하고, 의도치 않은 NULL이 존재하지 않아야 함.
 * */
...
-- 실무에서 VIEW를 많이 사용하지는 않음. 애초에 권한부여를 잘 안 하는 편.
...
INCREMENT BY 5 	-- NEXTVAL 호출된 후부터 5씩 증가
...
--> 현재 시점에서 시퀀스 생성후 NEXTVAL을 호출한 적이 없어서
--> 반환할 값이 없으므로 에러가 발생함.
...
SELECT SEQ_TEST_NO.CURRVAL FROM DUAL; -- 100 반환됨.

-- NEXTVAL을 호출할 때마다 
-- INCREMENT BY에 작성된 수 만큼 증가하는지 확인해보기
SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL;
-- 처음에는 100 나왔었음.
-- 1회 : 105
-- 2회 : 110
...
-- TB_TEST 테이블에 PK값을 SEQ_TEST_NO 시퀀스로 생성해보자
INSERT INTO TB_TEST 
...
FROM EMPLOYEE;
-- ROWID : 오라클에서 각 행의 고유한 주소를 나타내는 가상 컬럼(무리적 주소)
-- 인덱스가 ROWID 저장함.
-- 인덱스는 컬럼의 값과 해당 행의 ROWID를 매핑해서 저장함.
-- 컬럼값 -> 해당 행의 ROWID를 빠르게 찾아낼 수 있다!

-- 현재 사용자에 생성된 인덱스 목록 조회해보기
SELECT INDEX_NAME, TABLE_NAME, UNIQUENESS, STATUS
FROM USER_INDEXES;

-- EMPLOYEE 테이블에 EMP_NAME 컬럼에 인덱스 생성해보기
CREATE INDEX IDX_EMP_NAME 
ON EMPLOYEE (EMP_NAME);
--> EMPLOYEE 테이블의 EMP_NAME 컬럼에 대해 빠른 검색이 가능해짐.
...
-- UNIQUE INDEX 는 중복 방지 역할
-- EMAIL 컬럼에 중복되지 않는 고유한 값만 허용
--> 중복된 EMAIL 삽입 시 오류가 발생하게 된다.

----------------------------------------------------------

-- 인덱스 성능 확인용 테이블 생성
CREATE TABLE TB_IDX_TEST (
	TEST_NO NUMBER PRIMARY KEY,	--PK 제약조건 시 자동으로 UNIQUE INDEX 생성됨.
...
-- * PL/SQL 이란?
-- 오라클 데이터베이스에서 사용하는 절차적 확장 언어이다.
-- 변수, 조건문(IF/CASE), 반복문(LOOP, WHILE) 사용 가능.
BEGIN 
	FOR I IN 1..1000000	-- I는 1부터 100만까지 반복하겠다는 의미
	LOOP 
		INSERT INTO TB_IDX_TEST	-- TB_IDX_TEST테이블에 삽입할거다
		VALUES(I, 'TEST' ||	I);	-- || : 이어쓰기 ==> 1, TEST1   ... 1000000, TEST1000000
	END LOOP;
...
SELECT * FROM TB_IDX_TEST;	-- 이거 수행 시 컴퓨터가 힘들어할 수도 있음
...
WHERE TEST_ID = 'TEST500000';	-- 조회 시 걸린 시간: 0.015s

/*	인덱스 사용 시 검색 */
--TEST_NO가 500000인 행을 조회하기
SELECT * FROM TB_IDX_TEST
WHERE TEST_NO = 500000;	-- 조회 시 걸린 시간 :0.002s

-- 보통 조회 속도 10 ~ 30배 차이가 남.
