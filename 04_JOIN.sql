...
-- 사회, 이름, 부서코드는 EMPLOYEE 테이블에서 조회 가능
SELECT EMP_ID, EMP_NAME, DEPT_CODE
FROM EMPLOYEE;

-- EMPLOYEE 테이블 전체 컬럼 조회해봐도 부서명 컬럼은 없음
SELECT * FROM EMPLOYEE;

-- 부서명은 DEPARTMENT 테이블에서 조회 가능
SELECT * FROM DEPARTMENT; -- 부서명이 DEPT_TITLE이라는 거 확인 가능

-- EMPLOYEE 테이블의 DEPT_CODE와,
-- DEPARTMENT 테이블의 DEPT_ID를 연결고리 지정하여 JOIN하면 됨.
...
-- 작성하는 방법은 크게 ANSI 구문과 오라클 구문으로 나뉘고
-- ANSI에서 USING과 ON을 쓰는 방법으로 나뉜다.

-- JOIN만 작성할 경우 기본적으로 INNER JOIN으로 작동한다.
-- OUTER JOIN으로 사용하고 싶은 경우 반드시 OUTER JOIN을 명시해야 한다
...
-- DEPARTMENT 테이블의 LOCATION_ID, LOCATION 테이블의 LOCAL_CODE 겹침
...
-- ANSI 방식
-- 연결에 사용할 컬럼명이 같은 경우에 USING(컬럼명) 을 사용할 수 있다.
...
-- 테이블에 별칭을 줘서 어떤 테이블의 컬럼을 의미하는지 구분할 수 있도록 한다.
...
-- 오라클 구문 LEFT JOIN 작성법
SELECT EMP_NAME, DEPT_TLTLE
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+); 		-- 에러나는데 뭐냐
...
--> 개발자가 의도적으로 교차 조인을 사용하는 경우는 드물고, 
-- 주로 JOIN 구문을 잘못 작성하는 경우 CROSS JOIN의 결과가 조회됨
SELECT EMP_NAME, DEPT_TITLE
...
FROM EMPLOYEE
JOIN SAL_GRADE ON(SALARY BETWEEN MIN_SAL AND MAX_SAL); -- 비등가 조인
-- SQL Error [918] [42000]: ORA-00918: 열의 정의가 애매합니다
...
-- 테이블마다 별칭 작성 필수(미작성 시 열의 정의가 애매하다는 오류 발생)
...
-- 또는 JOIN JOB ON (EMPLOYEE.JOB_CODE = JOB.JOB_CODE)
...
SELECT EE.EMP_NAME, EE.EMP_NO, DD.DEPT_TITLE, JJ.JOB_NAME
FROM EMPLOYEE EE, DEPARTMENT DD, JOB JJ
WHERE EE.DEPT_CODE = DD.DEPT_ID
AND EE.JOB_CODE = JJ.JOB_CODE
AND EMP_NO LIKE '7%'
--AND TO_NUMBER(SUBSTR(EE.EMP_NO, 1, 2)) BETWEEN 70 AND 79 -- 주민번호가 70년대생
AND SUBSTR(EE.EMP_NO, 8, 1) = '2' -- 여자
AND EE.EMP_NAME LIKE '전%';	-- 성이 '전'씨
-- SUBSTR 보는법 - SUBSTR(EE.EMP_NO, 8, 1)
-- 770808-2665412 
-- 12345678	 ==> 8번째에 글자에 해당하는 2를 기준으로 봄. 2에서부터 1글자만 잘라냄 ==> '2'
...
SELECT EMP_ID, EMP_NAME, JJ.JOB_NAME, DD.DEPT_TITLE
FROM EMPLOYEE EE, JOB JJ, DEPARTMENT DD
WHERE EE.JOB_CODE = JJ.JOB_CODE
AND EE.DEPT_CODE = DEPT_ID
AND EMP_NAME LIKE '%형%';
...
FROM EMPLOYEE FULL
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID) FULL
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
WHERE BONUS IS NOT NULL; -- 보너스를 받는 직원
...
FROM EMPLOYEE EE, JOB JJ, DEPARTMENT DD, LOCATION LL
WHERE EE.JOB_CODE = JJ.JOB_CODE
...
-- 급여등급별 최소급여(MIN_SAL) ==> GROUP BY JOB_CODE, MIN_SAL : 직급별(JOB_CODE)로 그룹 나누고, 그 그룹 내에서 최소급여(MIN_SAL)로 그룹화

SELECT EMP_NAME, JOB_NAME, SALARY, SALARY * 12 * (NVL(BONUS, 0) + 1) 연봉
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
JOIN SAL_GRADE USING (SAL_LEVEL)	
GROUP BY JOB_CODE, MIN_SAL, EMP_NAME, JOB_NAME, SALARY, BONUS
HAVING SALARY > MIN_SAL	-- 최소 급여 초과
ORDER BY JOB_CODE;	-- 직급 오름차순 정렬 
...
-- 한국(KO)과 일본(JP)에 근무하는 직원들 -> 그룹화인가????
SELECT EMP_NAME 사원명, DEPT_TITLE 부서명, LOCAL_NAME 지역명, NATIONAL_NAME 국가명
FROM EMPLOYEE 직원, DEPARTMENT 부서, LOCATION LO, NATIONAL NA
WHERE DEPT_CODE = DEPT_ID
AND LOCATION_ID = LOCAL_CODE
AND LO.NATIONAL_CODE = NA.NATIONAL_CODE
GROUP BY NATIONAL_NAME, EMP_NAME, DEPT_TITLE, LOCAL_NAME
HAVING NATIONAL_NAME IN ('한국', '일본');
...
FROM EMPLOYEE EMP
JOIN EMPLOYEE EMP2 ON (EMP.DEPT_CODE = EMP2.EMP_NAME)
GROUP BY DEPT_CODE, EMP.EMP_NAME, EMP2.EMP_NAME;
