...
-- DTRL + ALT + 위아래방향키 : 줄 위아래 복사

-- SQL(Structured Query Language, 구조적 질의 언어)
-- 데이터베이스와 상호작용을 하기 위해 사용하는 표준 언어이다.
-- SPL을 이용하여 데이터의 조회, 삽입, 수정, 삭제 등을 수행할 수 있다.
...
--> 조회하겠다 컬럼명을 테이블명에서 => 테이블의 특정 컬럼을 조회하겠다.
...
-- EMPLOYEE 테이블에서 모든 사원의 사번, 이름, 급여, 연봉 조회해보기
...
-- 더미테이블DUAL(DUmmy tAbLe): 실제 데이터를 저장하는게 아닌, 임시 계산이나 테스트 목적 사용

-- 문자열 타입이어도 저장된 값이 숫자라면 자동으로 형변환하여 연산 가능
...
-- EMPLOYEE 테이블에서 이름, 입사일, 오늘 날짜 조회해보기
SELECT EMP_NAME, HIRE_DATE, SYSDATE FROM EMPLOYEE;
-- 2025-03-07 15:48:21.000
...
-- 컬럼 별칭 지정
/*
 * 컬럼명 AS 별칭 : 별칭에 띄어쓰기 x, 특수문자 x , 문자만 O
 * 
 * 컬럼명 AS "별칭" : 별칭 띄어쓰기 O, 특수문자 O, 문자만 입력하는 것도 O
 *  
 * AS는 생략 가능
 * 
 * */
...
-- DISTINCT : 조회 시 컬럼에 포함된 중복 값을 한 번만 표기할 때 사용
-- 주의사항 1) DISTINCE 구문은 SELECT 마다 딱 한 번 씩만 작성 가능하다
-- 주의사항 2) DISTINCT 구문은 SELECT 제일 앞에 작성되어야 한다.
SELECT DISTINCT DEPT_CODE, JOB_CODE FROM EMPLOYEE;
-- DEPT_CODE와 JOB_CODE를 모두에 해당하는 중복값이 없게끔 나온다.
-- 수행해보면 1번열에 D9이면서 J2와 중복되는 값을 가진 컬럼이 없는 걸 확인할 수 있다.
...
-- NOT 연산자 사용 가능 BETWEEN 앞에 작성 => 범위에 해당하는 값 제외
SELECT EMP_ID, EMP_NAME, SALARY, PHONE
FROM EMPLOYEE
...
FROM EMPLOYEE
WHERE EMAIL LIKE '서브쿼리_%';
-- 안됨. LIKE 에서 사용하는 _는 패턴으로서만 인식됨.
...
-- EMPLOYEE 테이블에서 - (FROM)
-- 이메일 '_'앞이 4글자 이면서 - (WHERE)
-- 부서코드가 'D9' 또는 'D6'이고 - (WHERE, OR)
-- 입사일이 1990-01-01 ~ 2000-12-31 사이이고 - (WHERE, BETWEEN)
-- 급여가 270만원 이상인 사원의 - (WHERE, >=)
-- 사번, 이름, 이메일, 부서코드, 입사일, 급여 조회 (SELECT)
----- tip: AND가 OR보다 우선순위가 높음. 우선순위로 사용할 내용을 ()안에 작성할 수 있음.
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE NOT IN('D1', 'D6', 'D9') -- 여기까지만 작성했을 시 12명 조회됨(NULL 미포함)
-- EMPLOYEE 총 23명이 있는데 조회되지 않는 나머지 인원은 부서코드가 NULL
OR DEPT_CODE IS NULL; -- 부서코드가 없는(NULL) 2명도 포함해서 14명 조회됨.
...
FROM EMPLOYEE
ORDER BY SALARY; -- ASC(오름차순)가 기본값. ASC를 작성하지 않을 경우 기본으로 오름차순 정렬. 내림차순 -> DESC
-- ODRDER BY 컬럼명, 별칭, 컬럼 순서 작성 가능.
...
ORDER BY 3 DESC; -- ORDER BY 컬럼 순서

-- 입사일 순서대로 이름. 입사일 조회(별칭 사용)
SELECT EMP_NAME 이름, HIRE_DATE 입사일
FROM EMPLOYEE
ORDER BY 입사일; 
-- ** WHERE절이 있을 경우 해석 순서에 의해 ORDER BY절에서 별칭 사용 불가! **
