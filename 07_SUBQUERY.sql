 ...
 * SUBQUERY (서브쿼리 == 내부쿼리)
 * 
 * - 하나의 SQL문 안에 포함된 또 다른 SQL문을 말함.
 * - 메인 쿼리(== 외부쿼리, 기존쿼리)를 위해 보조 역할을 하는 쿼리문이다.
 * 
 * -- 메인쿼리가 SELECT 문일 때
...
-- 부서코드가 노옹철 사원과 같은 부서 소속인 직원의
-- 이름, 부서코드 조회 => 메인쿼리

-- 1) 노옹철 사원의 부서코드를 조회해봐야함. => 서브쿼리
SELECT DEPT_CODE
FROM EMPLOYEE
WHERE EMP_NAME = '노옹철';	-- 부서코드가 D9인 것을 확인
...
FROM EMPLOYEE
-- WHERE SALARY >= CEIL(AVG(SALARY)); ==> WHERE절에서 집계함수 사용 불가
-- 대신 HAVING 절을 사용해야 함
WHERE SALARY >= 3047663;

-- 3) 위의 2 단계를 하나의 쿼리로 합치기
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
 ...
 * 						서브쿼리의 결과값도 바뀌는 서브쿼리
 * 
 * - 스칼라 서브쿼리 : 상관 쿼리이면서 결과 값이 하나(단일행)인 서브쿼리
 * 
 * ** 서브쿼리 유형에 따라 서브쿼리 앞에 붙는 연산자가 다름 ** 
 * 
 * */	
-- 단일행은 비교연산자 사용 가능
-- 다중행은 비교연산자 사용 불가. 대신 IN 연산자 사용가능

--------------------------------------------------------------------

-- 1. 단일행 서브쿼리 (SINGLE ROW SUBQUERY)
-- 서브 쿼리의 조회 결과 값의 개수가 1개인 서브쿼리를 말한다.
-- 단일행 서브쿼리 앞에는 비교 연산자 사용할 수 있다.
-- 비교 연산자 : <, >, <=. >=, =, (!= / <> / ^=) 
...
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)  -- DEPT_CODE가 없는 직원도 조회해올 것이므로 EMPLOYEE테이블의 모든 행을 기준으로 LEFT JOIN
WHERE SALARY > (SELECT AVG(SALARY) FROM EMPLOYEE) -- SELECT AVG(SALARY) FROM EMPLOYEE: 단일행 서브쿼리=> 비교연산자 가능 
ORDER BY JOB_CODE; -- SELECT 절에 명시되지 않은 컬럼이라도
...
-- 부서별(부서가 없는 사람 포함)로 급여의 합계 중
-- 가장 큰 부서의 부서명, 급여, 합계를 조회

-- 1) 부서별 급여 합 중 가장 큰 값 조회(단일행 서브쿼리)
...
-- 5) 3, 4 의 조회 결과를 하나로(사수, 사원 둘 다) 조회

-- 1. 집합 연산자(UNIOM 합집합) 사용하기
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME, '사수' 구분
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
...
-- 2) 선택함수 사용
--> DECODE();
--> CASE WHEM 조건1 THEN 값1 .. ELSE END
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME, 
...
-- 방법1)  > ANY : 가장 작은 값보다 크냐? 활용해서 풀어보는 방법
...
--	< ALL : 가장 작은 값보다 작은가?
--==> 차장 직급의 최소 급여보다 적제 받는 과장인가?
							

-- 서브쿼리 중첩 사용 : 메인쿼리 안에 서브쿼리 안에 서브쿼리 (응용편!)
...
-- 1) LOCATION 테이블에서 NATIONAL_CODE가 KO인 경우의 LOCAL_CODE 조회 (서브쿼리)
SELECT LOCAL_CODE
FROM LOCATION
...
-- (단일행) 다중열 서브쿼리
-- : 서브쿼리 SELECT 절에 나열된 컬럼수가 여러 개 일 때
...
-- 컬럼 순서가 중요!!		/*아래 주석/**/ 안 내용과 같은 말임*/
SELECT EMP_NAME, JOB_CODE, DEPT_CODE, HIRE_DATE
FROM EMPLOYEE
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE
		/*괄호 안의 컬럼 작성 순서는*/	 FROM EMPLOYEE
		/*서브쿼리 조회 결과 컬럼 순서와*/	WHERE ENT_YN = 'Y'
		/*동일해야함.*/		 					  AND SUBSTR(EMP_NO, 8, 1) = '2');														
...
AND EMP_NAME = '노옹철';				-- D9, 부사장

-- 노옹철 사원과 같은 부서(D9), 같은 직급(부사장)인 사원
SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE, DEPT_TITLE, JOB_NAME
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
...
-- 메인쿼리의 테이블 값이 변경되면 서브쿼리도 영향을 받아서 서브쿼리의 결과값도 바뀌게 되는 구조
...
-- 서브쿼리
-- 메인쿼리의 1행에 해당하는 서브쿼리(선동일의 평균 급여 조회)를 수행
SELECT AVG(SALARY)
FROM EMPLOYEE
WHERE JOB_CODE = 'J1';	-- 80,00,000	-- 평균과 같음 => 최종 RESULT SET에서 제외되어야 함.

SELECT AVG(SALARY)
FROM EMPLOYEE
WHERE JOB_CODE = 'J2'; -- 4,850,000 -- 송중기의 급여가 J2의 급여평균보다 높음, 노옹철은 낮음.
..
...
-- 상관쿼리
SELECT EMP_NAME, JOB_CODE, SALARY
...
WHERE SALARY > (SELECT AVG(SALARY)
	...
	-- 테이블에 별칭 붙여서 서브쿼리와 메인쿼리의 테이블을 구분할 수 있게 하고, 직급코드를 별칭.컬럼명으로 변경하면
-- 메인쿼리의 한행 한행에 해당하는 서브쿼리가 수행됨.
...
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- 부서코드가 없는 것도 조회
...
-- 직급명, 입사일을 조회, 단, 퇴사한 직원 제외)
SELECT EMP_ID, EMP_NAME, DEPT_CODE, NVL(DEPT_TITLE, '소속없음'), 
JOB_NAME, HIRE_DATE
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_ID = EMPLOYEE.DEPT_CODE);
WHERE ENT_YN = 'N';

-- 서브쿼리(부서별 입사일이 가장 빠른 사원)
SELECT MIN(HIRE_DATE)
FROM EMPLOYEE
WHERE DEPT_CODE = 'D1'; -- 2007-03-20 00:00:00.000	- 박명수 제외

-- D8 부서의 가장 빠른 입사일 : 1997-09-12 00:00:00.000
-- 메인쿼리에서 퇴사자이면서 D8부서의 가장 빠른 입사일인 이태림 제외시킨 상태
...
-- D8부서가 안나옴. -> 퇴사자인 이태림이 D8인데 그 사람이 가장 입사일이 빨라서. 
-- 이태림 제외하고 D8부서에서 입사일이 빠른 직원 조회되도록 작성		
...
-- ** 인라인뷰의 RESULT SET 결과의 컬럼과 메인 쿼리의 RESULT SET 결과의 컬럼이 같아야함.
...
-- 인라인뷰를 활용한 TIP-N분석해보기 

-- 전 직원 중 급여가 높은 상위 5명의
-- 순위, 이름, 급여 조회

-- ROWNUM 컬럼: 헹 번호를 나타내는 가상 컬럼
...
WHERE ROWNUM <= 5;	-- 해석순위 2위. WEHRE 절에서 가상 컬럼의 1~5행까지만 조회
...
-- EX) 공동 1위가 2명이라면, 다음 순위는 2위가 아닌 3위
...
-- 19	윤은해	2000000
-- 20	박나라	1800000
-- 박나라 순위가 건너뛰어지지 않고 바로 다음 순위로 나옴.


------------------------------------------------------
-- PDF 06_SUBQUERY_실습문제.pdf
...
AND EMP_NAME = '전지연';	-- 부서코드 : D1	/ 부서명 : 인사관리부

-- 2) (전지연이랑 같은 부서인) 
-- 부서원들의 사번, 사원명, 전화번호, 고용일, 부서명 조회 
-- (단 전지연 제외)
SELECT * FROM EMPLOYEE;
...
WHERE (DEPT_ID, DEPT_TITLE) = (SELECT DEPT_ID, DEPT_TITLE -- = 대신 IN도 됨.
FROM DEPARTMENT, EMPLOYEE
WHERE DEPT_ID(+) = DEPT_CODE
AND EMP_NAME = '전지연')
AND EMP_NAME <> '전지연';
...
-- 고용일이 2000년도 이후인 사원들 중 급여가 가장 높은 사원
SELECT MAX(SALARY)
FROM EMPLOYEE
